...
try:
    ...
    # 取第一只候选股测试打分
    first_sector = list(config.SECTOR_CANDIDATES.keys())[0]
    first_stocks = config.SECTOR_CANDIDATES[first_sector].get('stocks', {})
    
    if first_stocks:
        test_code = list(first_stocks.keys())[0]
        test_stock = first_stocks[test_code]
        
        # 调用 canslim_score 函数
        score = canslim_score(test_code, test_stock)
        
        if score and 'factor_score' in score:
            result.add_pass("CANSLIM", "六因子打分", f"{test_code}评分：{score['factor_score']}")
        else:
            result.add_fail("CANSLIM", "六因子打分", f"{test_code}评分结果异常：{score}")
    else:
        result.add_skip("CANSLIM", "六因子打分", "无候选股")
except Exception as e:
    result.add_fail("CANSLIM", "六因子打分", f"异常：{e}")

# 测试 1.3: 买点计算
print("\n[测试 1.3] 买点计算（三档买点 + 止损价）")
try:
    ...
    if first_stocks:
        ...
        # FIX(2026-08-14): calculate_buy_plan 新签名 (df, code, factor_result, market_info=None)
        # 构造含必要列的小型 DataFrame（合理常量序列，不依赖网络）
        _n = 30
        _test_df = pd.DataFrame({
            'close': [10.0] * _n, 'high': [10.2] * _n, 'low': [9.8] * _n,
            'ma5': [10.0] * _n, 'ma10': [10.0] * _n, 'ma20': [9.9] * _n,
            'ma60': [9.8] * _n, 'volume': [100000] * _n, 'atr': [0.1] * _n,
        })
        # factor_result 最小可用 dict（参照 stock_screener 内实际读取的键）
        factor_result = {'total_score': 50, 'factors': {}, 'reason': '测试', 'signals': []}
        market_info = {'can_buy': True, 'position_limit_ratio': 1.0}
        
        # 调用 calculate_buy_plan
        buy_plan = calculate_buy_plan(_test_df, test_code, factor_result, market_info)
        
        required_fields = ['aggressive_buy', 'moderate_buy', 'conservative_buy', 'stop_loss']
        missing = [f for f in required_fields if f not in buy_plan]
        
        if not missing:
            result.add_pass("CANSLIM", "买点计算", f"{test_code}三档买点完整")
        else:
            result.add_fail("CANSLIM", "买点计算", f"缺少字段：{missing}")
    else:
        result.add_skip("CANSLIM", "买点计算", "无候选股")
except Exception as e:
...
try:
    ...
    import caopan_report
    
    screener_result = caopan_report.run_screener()
    
    if screener_result and 'stock_pool' in screener_result:
        stock_pool = screener_result['stock_pool']
        recommended = [s for s in stock_pool if s.get('is_buy_recommend')]
        
        if len(recommended) > 0:
            # 检查推荐标的是否包含完整字段
            first_rec = recommended[0]
            required = ['code', 'name', 'aggressive_buy', 'moderate_buy', 'conservative_buy', 
                       'stop_loss', 'first_shares', 'first_amount', 'factor_score']
            missing = [f for f in required if f not in first_rec]
            
            if not missing:
                result.add_pass("CANSLIM", "选股结果数据质量", f"推荐{len(recommended)}只，字段完整")
            else:
                result.add_fail("CANSLIM", "选股结果数据质量", f"缺少字段：{missing}")
        else:
            result.add_skip("CANSLIM", "选股结果数据质量", "无推荐标的")
    else:
        result.add_fail("CANSLIM", "选股结果数据质量", "选股结果为空")
except Exception as e:
...
try:
    ...
    # FIX(2026-08-14): calculate_buy_plan 新签名，且返回不再含 is_watch/is_buy_recommend。
    # 大盘禁买语义(is_watch=True)在 run_stock_screener 出口层面设置
    # （buy_plan["is_watch"] = not market_info["can_buy"]）；函数内部仅通过
    # market_info["position_limit_ratio"] 影响资金——禁买时该值为0，
    # 有效资金归零→首批无法买入整手→计划降级。故此处验证降级/返回行为。
    _n = 30
    _test_df = pd.DataFrame({
        'close': [10.0] * _n, 'high': [10.2] * _n, 'low': [9.8] * _n,
        'ma5': [10.0] * _n, 'ma10': [10.0] * _n, 'ma20': [9.9] * _n,
    ...
    factor_result = {'total_score': 50, 'factors': {}, 'reason': '测试', 'signals': []}
    market_info = {'can_buy': False, 'position_limit_ratio': 0.0}  # 大盘禁买（仓位限制归零）
    
    result_plan = calculate_buy_plan(_test_df, '000001', factor_result, market_info)
    
    if result_plan.get('plan_degraded') is True and result_plan.get('first_shares', 1) == 0:
        result.add_pass("CANSLIM", "大盘禁买降级", "仓位限制归零→首批无法买入→计划降级")
    else:
        result.add_fail("CANSLIM", "大盘禁买降级", f"plan_degraded={result_plan.get('plan_degraded')}, first_shares={result_plan.get('first_shares')}")
except Exception as e:
    result.add_fail("CANSLIM", "大盘禁买降级", f"异常：{e}")

# 测试 1.6: 数据降级容错
print("\n[测试 1.6] 数据降级容错")
try:
    # 测试空数据输入
    from strategy.stock_screener import SECTOR_CANDIDATES
    
    if SECTOR_CANDIDATES is not None:
        result.add_pass("CANSLIM", "数据降级容错", "候选池加载未崩溃")
    else:
        result.add_fail("CANSLIM", "数据降级容错", "候选池为 None")
except Exception as e:
...
try:
    ...
    report_result = ghr.generate_holdings_report()
    
    if report_result and 'html' in report_result:
        html_content = report_result['html']
        
        # 检查关键章节
        sections = ['持仓分析', '选股候选', '市场情绪', '条件单', '风控行动项']
        found_sections = [s for s in sections if s in html_content]
        
        if len(found_sections) >= 3:
            result.add_pass("综合报告", "报告生成完整性", f"包含{len(found_sections)}个关键章节")
        else:
            result.add_fail("综合报告", "报告生成完整性", f"仅包含{found_sections}")
    else:
        result.add_fail("综合报告", "报告生成完整性", "报告生成失败")
except Exception as e:
    result.add_fail("综合报告", "报告生成完整性", f"异常：{e}")
...
try:
    # 检查操盘密码相关函数
    from strategy.stock_screener import config
    
    # 检查是否有操盘密码相关配置
    has_caopan = hasattr(config, 'CAOPAN_PASSWORD_ENABLED') or hasattr(config, 'DK_SIGNAL_ENABLED')
    
    if has_caopan:
        result.add_pass("操盘密码", "DK 信号生成", "配置存在")
    else:
        result.add_skip("操盘密码", "DK 信号生成", "配置未定义（可能集成在综合报告中）")
except Exception as e:
    result.add_fail("操盘密码", "DK 信号生成", f"异常：{e}")

# 测试 3.2: 控盘生命线（MA20/MA60）
print("\n[测试 3.2] 控盘生命线（MA20/MA60 趋势判断）")
try:
    from strategy.stock_screener import config
...
print("\n[测试4.2] 盘中检测逻辑 - 非交易时段跳过")
try:
    from notify.buy_point_alert import check_buy_point_alerts, _in_trading_hours
