...
# 测试1.2: 选股结果数据质量（检查现有buy_alert_levels.json）
print("\n[测试1.2] 选股结果数据质量")
try:
...
print("\n[测试2.4] 报告生成文件存在性")
try:
    # generate_holdings_report.py 在项目根目录，不在 trading_system 子目录
    report_file = os.path.join(config.PROJECT_ROOT, '..', 'generate_holdings_report.py')
    report_file = os.path.abspath(report_file)
    
    if os.path.exists(report_file):
        result.add_pass("综合报告", "报告生成文件", "generate_holdings_report.py存在")
    else:
        result.add_fail("综合报告", "报告生成文件", "文件不存在")
except Exception as e:
    result.add_fail("综合报告", "报告生成文件", f"异常：{e}")
...
# 测试3.1: 条件单价格（Ratchet原则）
print("\n[测试3.1] 条件单价格（止损价Ratchet原则）")
try:
    ratchet_violations = []
    for code, h in active_holdings.items():
...
# 测试4.2: 同档去重机制
print("\n[测试4.2] 同档去重机制（buy_point_alert_sent.json）")
try:
